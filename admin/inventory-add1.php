    ...
    // The same dapat ang input name sa Add kag Update.....WHAT THE MEN!!! 
     ...
     if (isset($_POST['submit'])) {
		...
		$rack = $_POST['rack'];
			...
			if ($checkprod > 0) {
              header("Location: inventory-add.php?required=producttaken");    
            }elseif ($product == "") {
              header("Location: inventory-add.php?required=product");
            }elseif ($quantity == "" || $quantity < 0 ) {
              header("Location: inventory-add.php?required=quantity");    
            }elseif ($price == "" || $price < 0 ) {
              header("Location: inventory-add.php?required=price");  
            }elseif ($partname == "" || $partname < 0 ) {
              header("Location: inventory-add.php?required=partname");  
            }else{
				$date = $_POST['date'];
				$user = $_POST['user'];
            	$query = "INSERT INTO `particulars`(`part_name`, `part_code`, `price`, `quantity`,`rack_id`,`stk_id`, `status`)
				VALUES ('".$partname."','".$product."','".$price."','".$quantity."','".$rack."','".$user."','Available')";
